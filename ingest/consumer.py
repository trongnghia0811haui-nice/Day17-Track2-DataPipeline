...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Upsert theo event_id để replay do at-least-once không tạo duplicate và vẫn
    cập nhật nội dung nếu message được phát lại đã thay đổi.
    """
    con.executemany(
        f"""
        insert into {TABLE}
    ...
    # Đảm bảo dữ liệu đã bền vững trước khi offset được commit.
    con.commit()
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            maybe_crash(batch_no, crash_at)   # sự cố xảy ra trước commit offset
